...
export function TranscriptPanel({
  ...
  const { transcripts, transcriptContainerRef, copyTranscript, currentMeetingId } =
    useTranscripts();
  const { transcriptModelConfig, showConfidenceIndicator } = useConfig();
      ...
      {transcripts?.length > 0 && (
        <div className="px-4 pt-4">
          <p
            ...
            "
          >
            <span className="font-medium text-foreground">Live draft.</span>{" "}
            Transcribed in 30-second windows as you speak, so sentences get cut
            at window boundaries. When the recording ends, use{" "}
            <span className="font-medium">Rebuild transcript</span> on the
            meeting page for the accurate version, made from the whole audio.
          </p>
        </div>
      )}
